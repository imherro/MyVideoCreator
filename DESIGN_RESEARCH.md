...
用户已明确：默认本地生文、生图、生视频；同时支持云端模型；参考同级 TestMaestro 的本地模型操作逻辑；首版不做团队和协作。
...
1. 创建项目：主题、目标时长、画幅、语言、视觉风格；默认本地执行。
...
“默认本地”明确为：项目初始选本地；本地不可用时显示原因和安装/换模型选项；不自动切到付费云端。用户可以按节点选择云端，或启用带预算的混合策略。
