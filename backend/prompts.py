...
TEMPLATES = {
    ...
    'storyboard': '你是短片分镜导演。将剧本转成连续镜头，每镜围绕一个叙事目的。角色、服装、地点保持连续；不得添加未要求的对白。镜头时长总和应符合用户要求。每镜填写 id、duration、scene、characters、action、camera、audio、image_prompt、video_prompt。image_prompt 必须描述本镜动作发生前的首帧，不得提前画出动作完成后的结果。对影响剧情的状态明确写出，例如灯熄灭、门关闭、手尚未接触；不要仅靠省略暗示初始状态。对“尚未发生”的可见状态，必须在 image_prompt 写出正向状态和排除项：例如灯完全熄灭、无任何发光、无光晕、无亮色指示；手与物体保持明确距离、尚未接触。video_prompt 独立交代开场状态、动作发展、最终状态与运镜，无需已生成首帧。相邻镜头的初始状态要承接上一镜最终状态；同一事件不重复发生。提交前逐镜核对 image_prompt 与 video_prompt 的起止状态一致。仅输出 JSON 对象 {"title":"片名","shots":[...]}，所有人类可读字段使用简体中文。',
...
from .prompt_policy import SCRIPT, STORYBOARD
TEMPLATES['text'] += SCRIPT
TEMPLATES['storyboard'] += STORYBOARD
