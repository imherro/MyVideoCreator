...
有明确分集标题时严格使用给出的规则边界和集号，不得把镜头、场景或小标题拆成新集。无标题时可以按已有剧情结构建议边界；不是剧本或无法可靠分集则 episodes 为空，并解释原因。
...
def extract(content, start, end):
    return ''.join(source_lines(content)[start-1:end])


def rule_manifest(content, filename):
    ...
    for index,(start,match) in enumerate(heads):
        ...
        body = extract(content,start,end)
    ...
    if not rows: result['warnings'].append('未识别到明确集标题，请使用 AI 识别，或作为原著资料导入。')
    return result


def validate_manifest(content, value, rules):
    ...
    lines = source_lines(content); rows=value['episodes']
...
def public_draft(connection, row):
    ...
    for ep in manifest['episodes']:
        ...
        body=extract(content,ep['startLine'],ep['endLine'])
    ...
    prefix=extract(content,1,episodes[0]['startLine']-1) if episodes else ''
...
def analysis_prompt(row):
    return '标题识别结果：'+row['manifest']+'\n以下每行以行号开头，行号不是原文：\n'+'\n'.join(f'{i+1}: {line.rstrip()}' for i,line in enumerate(source_lines(row['content'])))
...
def apply_analysis(job, value):
    ...
    with s.db() as c:
        ...
        result=validate_manifest(row['content'],value,rule_manifest(row['content'],row['filename']))
...
def confirm_import(connection, production_id, import_id, episode_nos, include_shared=True):
    ...
    if row['status']=='imported':return json.loads(row['result'])
    ...
    manifest=json.loads(row['manifest']);selected=[x for x in manifest['episodes'] if x['episodeNo'] in episode_nos]
    ...
    for index,ep in enumerate(manifest['episodes'],1):
        ...
        connection.execute('INSERT INTO source_chapters VALUES(?,?,?,?,?,?,?,?,?)',(cid,source_id,index,f'第{ep["episodeNo"]}集：{ep["title"]}',extract(row['content'],ep['startLine'],ep['endLine']),index,1,now,now))
    prefix=extract(row['content'],1,manifest['episodes'][0]['startLine']-1)
    ...
    for ep in selected:
        ...
        save_script_row(connection,script,{'title':ep['title'],'synopsis':'','body':extract(row['content'],ep['startLine'],ep['endLine']),'estimatedDuration':doc['duration'],
