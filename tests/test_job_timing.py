...
def test_telemetry_and_terminal_time_persist_and_late_events_do_not_overwrite():
    ...
    with s.db() as c:
        c.execute('INSERT INTO projects(id,name,revision,document,created,updated) VALUES(?,?,1,?,?,?)',(pid,'Timing','{}',now,now))
    ...
    with s.db() as c:job=s.unpack(c.execute('SELECT * FROM jobs WHERE id=?',(jid,)).fetchone())
    ...
    with s.db() as c:assert s.unpack(c.execute('SELECT * FROM jobs WHERE id=?',(jid,)).fetchone())['finished']==job['finished']
