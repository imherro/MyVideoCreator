...
def test_compiler_uses_only_asset_bindings_in_character_scene_prop_order():
    ...
    assert '色板：冷蓝' in result['prompt'] and '布光：路灯侧逆光' in result['prompt']
    ...
    assert '外观：林岚的固定外观' in result['prompt']
    assert '属性：颜色：hero-color' in result['prompt']
    assert '不可改变：林岚不可改变' in result['prompt']
    assert '拼贴' in result['prompt']
