...
export function ModelSelector({data,providers,localModels,request,onChange}:{data:Value;providers:Value[];localModels:Value[];request:(path:string)=>Promise<any>;onChange:(patch:Value)=>void}){
 ...
 return <><label>模型服务<select value={providerId} onChange={e=>{const p=providers.find(p=>p.id===e.target.value);onChange({provider:e.target.value,model:p?.models?.[kind]||p?.model||localModels[0]?.id||'',model_capabilities:undefined})}}>
