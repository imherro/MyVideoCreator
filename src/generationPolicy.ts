...
export function resolveGenerationTarget(kind:GenerationKind,override:Value|undefined,policy:GenerationPolicy|undefined,providers:Value[],localModels:Value[]=[]){
 ...
 if(target){
  ...
  if(!provider)throw new Error(`${source==='project'?'项目默认':'节点自定义'}模型服务已不存在，请重新选择；系统不会自动切换到其他收费模型`);
 ...
 const provider=providers.find(p=>p.local&&p.kind===kind)||providers.find(p=>p.kind===kind);
