        ...
        <>
          ...
          <div className="asset-grid production-media-grid">
            {media.map((asset) => <article className="asset-card" key={asset.id}>
              ...
              <div>{asset.kind === "image" && <button onClick={() => onPanorama(asset)}>全景取景</button>}{asset.kind === "image" && onReference && <button onClick={() => onReference(asset)}>引用</button>}{["image", "video"].includes(asset.kind) && <button onClick={() => onTimeline(asset)}>入时间线</button>}<a href={asset.url} download={asset.name} title={`下载原文件：${asset.name}`}><Download size={14} />{asset.kind === "video" ? "下载原视频" : "下载"}</a><button className="icon-button danger asset-delete-button" onClick={() => onDelete(asset)} title="移入回收站"><Trash2 size={14} /></button></div>
            </article>)}
          </div>
