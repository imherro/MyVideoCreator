      ...
      return <article key={row.uid} className={`video-production-card ${selected.includes(row.uid) ? "selected" : ""}`}>
        ...
        <div className="video-production-main">
          ...
          <div className="video-production-fields">
            ...
            {!!row.shot.dialogues?.length && <details className="video-dialogue-projection"><summary>最终提交提示词 · 已自动加入 {row.shot.dialogues.length} 条对白</summary><pre>{compileVideoPrompt(row.shot.video_prompt,row.shot)}</pre></details>}
