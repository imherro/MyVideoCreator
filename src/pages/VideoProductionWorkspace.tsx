      ...
      return <article key={row.uid} className={`video-production-card ${selected.includes(row.uid) ? "selected" : ""}`}>
        ...
        <div className="video-production-main">
          ...
          <div className="video-production-fields">
            ...
            <div className="video-dialogue-projection" aria-label="实际发送给视频模型"><header><b>实际发送给视频模型</b><small>{row.shot.dialogues?.length ? `已自动加入 ${row.shot.dialogues.length} 条对白` : "本镜无结构化对白"}</small></header><pre>{compileVideoPrompt(row.shot.video_prompt,row.shot,row.effectiveDuration)}</pre>{row.effectiveDuration > row.plannedDuration && <small>固定对白超过卡片时长；提交时会把镜头从 {row.plannedDuration} 秒自动延长为 {row.effectiveDuration} 秒。</small>}{row.shot.dialogues?.length > 0 && ["volcengine_ark", "runninghub"].includes(row.provider?.type) && <small>{row.dialogueAudioAssets.length === row.shot.dialogues.length ? `固定音色对白已就绪 ${row.dialogueAudioAssets.length}/${row.shot.dialogues.length}；将作为 Seedance 2.5 音频参考提交。` : "请先在塑角造景生成当前音色版本的本镜对白。"}</small>}</div>
