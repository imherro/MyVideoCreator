    ...
    {!rows.length ? <div className="empty-state"><Film/><h3>还没有可生产的视频镜头</h3><p>请先在分镜工作区创建镜头，并准备所选生成模式需要的参考素材。</p></div> : !visible.length ? <div className="empty-state"><Film/><h3>当前筛选没有镜头</h3></div> : <div className="video-production-list">{visible.map((row) => {
      ...
      return <article key={row.uid} className={`video-production-card ${selected.includes(row.uid) ? "selected" : ""}`}>
        ...
        <div className="video-production-main">
          <div className="video-frame-column"><small>{videoGenerationMode(props.document,row.shot) === "multimodal" ? "构图参考 · 非硬首帧" : "FIRST FRAME"}</small><button className="video-frame-preview" disabled={!row.firstFrame} onClick={() => row.firstFrame && props.onPreview(row.firstFrame as Asset)}>{row.firstFrame ? <img src={row.firstFrame.url} alt={videoGenerationMode(props.document,row.shot) === "multimodal" ? "起始构图参考" : "首帧"}/> : <span><ImageIcon/>缺少首帧</span>}</button>{row.imageNode?.data?.stale && <em>首帧已过期</em>}{row.readinessReason && <p>{row.readinessReason}</p>}</div>
