import { dialogueModeLabels } from "../dialogueMode.ts";
  ...
  return <main className="task-detail-page">
    ...
    {!job ? <div className="loading task-detail-loading"><LoaderCircle className="spin"/>正在读取任务</div> : <>
      ...
      <section className="task-detail-card">
        ...
        {job.input?.visual_style_projection && <p className="muted">Bible 使用范围：{job.input.visual_style_projection.reason}</p>}
        {(job.input?.visual_style_projection?.warnings || []).map((warning:string)=><p role="alert" key={warning}>{warning}</p>)}
        ...
        {job.input?.source_event_extraction && runtimePromptStages.map((stage: Value) => <article className="prompt-stage" key={stage.id}>
          ...
          {(stage.attempts || []).map((attempt: Value, index: number) => <p key={index}>请求 {index + 1} · 输出上限 {attempt.max_tokens} tokens · 返回 {attempt.output_chars ?? "—"} 字符 · 结束原因 {attempt.finish_reason || (attempt.finished ? "服务未提供" : "等待返回")}</p>)}
          <details><summary>本段实际提示词</summary><pre className="debug-block">{stage.system_prompt}</pre><pre className="debug-block">{stage.user_prompt}</pre></details>
        </article>)}
        ...
        {job.input?.dialogue_mode && <><h3>对白生成方式</h3><p>{dialogueModeLabels[job.input.dialogue_mode.actual]} · {job.input.dialogue_mode.source === 'shot' ? '镜头覆盖' : '项目默认'}</p>{(job.input.voice_samples || []).map((sample:Value)=><p key={sample.characterCardId}>{sample.characterName} → 音频 {sample.index} · 声音 V{sample.voiceVersion} · {sample.source === "uploaded" ? "上传声音" : "豆包音色"} · 仅参考音色 · 样本 {sample.media?.duration} 秒（不决定镜头时长）</p>)}</>}
        {job.input?.video_ratio_selection && <><h3>视频画幅</h3><p>{job.input.video_ratio_selection.source === 'node' ? '本节点设置' : '继承项目'}：{job.input.video_ratio_selection.requested} · 实际提交：{job.input.video_ratio_selection.actual === 'first_frame' ? '跟随首帧' : job.input.video_ratio_selection.actual}</p></>}
