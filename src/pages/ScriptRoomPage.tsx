...
export function ScriptRoomPage({
  ...
  return <section className="script-room-page workflow-domain-page">
    ...
    {(importFilePreview||importResumeId)&&<ScriptImportDialog key={productionId} file={importFilePreview||undefined} resumeId={importResumeId} providers={providers} productionId={productionId} projectId={projectId} defaultTarget={defaultTarget} request={request} onJobsSubmitted={onJobsSubmitted} onClose={()=>{setImportFilePreview(null);setImportResumeId(undefined);}} onImported={async result=>{unsavedDrafts.current.clear();await onScriptsImported(result);await loadList(result.episodes[0]?.episodeNo);}}/>}
