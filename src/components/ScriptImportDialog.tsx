...
export function ScriptImportDialog({file,productionId,projectId,defaultTarget,request,onJobsSubmitted,onClose,onImported,onSourceImport}:{
  ...
  useEffect(()=>{
    ...
    void (async()=>{
      ...
      const value=await request(`${base}/upload`,{method:'POST',body});
      ...
      if(!value.manifest.episodes.length&&!value.job&&defaultTarget?.providerId)await submitAnalysis(value.id);
  ...
  async function confirm(){
    if(destination==='source'&&onSourceImport){await onSourceImport(draft!.id,selected);onClose();return;}
  ...
  return <div className="modal-overlay"><section className="script-import-dialog" role="dialog" aria-modal="true" aria-labelledby="script-import-title">
    <header><div><h2 id="script-import-title">{onSourceImport?'导入原著 / 剧本':'智能导入剧本'}</h2><small>{file.name}</small></div><button className="icon-button" aria-label="关闭导入预览" disabled={busy} onClick={onClose}><X size={18}/></button></header>
