...
export function CanvasVideoReferences({document,nodeId,assets,onCompile}:{document:Value;nodeId:string;assets:Value[];onCompile:()=>Promise<Value>}) {
 ...
 const signature=JSON.stringify([document.nodes.find((n:Value)=>n.id===nodeId)?.data,document.edges,visual,document.videoRatio,document.videoDuration,document.videoReferenceMode]);
