...
export const VIDEO_DURATION_MARKER = "[镜头时长]";

export function compileVideoPrompt(basePrompt: unknown, shot: Value, effectiveDuration?: number) {
  const base = String(basePrompt || "")
    .split(VIDEO_DURATION_MARKER, 1)[0]
    .split(VIDEO_DIALOGUE_MARKER, 1)[0]
    .trimEnd();
  const dialogues = (Array.isArray(shot.dialogues) ? shot.dialogues : [])
    .filter((item: Value) => String(item?.text || "").trim());
  const lines = [base];
  if (dialogues.length && !dialogues.every((item: Value) => base.includes(String(item.text).trim()))) {
    lines.push("", VIDEO_DIALOGUE_MARKER,
  ...
  const duration = Number(effectiveDuration ?? shot.duration);
  if (Number.isFinite(duration) && duration > 0) {
    lines.push("", VIDEO_DURATION_MARKER,
      `本镜头成片总时长必须为 ${duration} 秒；所有动作、运镜、开口与闭口必须在这段时间内完成。`);
  }
  return lines.join("\n").trim();
}
