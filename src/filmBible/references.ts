...
function promptFor(
  ...
  const lines = [
    card.kind.startsWith("character") ? "生成一张 1:1 正方形角色定妆设定板：上排三个面部特写（正面、四分之三侧面、纯侧面），下排四个完整全身视角（正面、四分之三侧面、侧面、背面）。同一角色同一服装状态，自然站立，头脚完整，浅色纯背景、均匀柔光，无文字水印。" : "生成一张影视视觉圣经主参考图。画面只用于固定后续镜头的一致性，不要添加文字、水印、分镜框或拼贴。",
