...
export function PromptLibrary({templates,request,newId,onApply}:{templates:Record<string,string>;request:(path:string,options?:RequestInit)=>Promise<any>;newId:()=>string;onApply:(kind:string,content:string)=>void}){
 ...
 useEffect(()=>{refresh()},[]);
 async function save(deleted=false){if(!draft)return;setBusy(true);try{const result=await request('/prompt-library/'+draft.id,{method:'PUT',body:JSON.stringify({...draft,revision:library.revision,deleted})});setLibrary(result);setDraft(null);setError('')}catch(e:any){setError(e.message)}finally{setBusy(false)}}
 return <><p className="muted">个人模板保存在主机，可跨电脑使用。剧本与分镜模板作为阶段规则；图像与视频模板填入创作描述。应用不会提交生成。</p>
 <div className="settings-actions"><button disabled={!ready} onClick={()=>setDraft({id:newId(),name:'新模板',kind:'text',content:'',version:0})}><Plus size={15}/>新建模板</button><button onClick={refresh}><RefreshCw size={15}/>刷新模板库</button></div>
 ...
 <h3>我的模板</h3>{library.templates.filter(t=>!t.deleted).map(t=><article className="template-card" key={t.id}><h3>{t.name} <small>v{t.version} · {names[t.kind]}</small></h3><p>{t.content}</p><button onClick={()=>onApply(t.kind,t.content)}>应用</button><button onClick={()=>setDraft({...t})}>编辑 / 历史</button></article>)}
 {!library.templates.some(t=>!t.deleted)&&<p className="muted">从下方内置模板复制，或创建自己的模板。</p>}
 {library.templates.some(t=>t.deleted)&&<details><summary>已归档模板</summary>{library.templates.filter(t=>t.deleted).map(t=><button key={t.id} onClick={()=>setDraft({...t})}>{t.name} · 编辑并恢复</button>)}</details>}
 <h3>内置模板</h3>{Object.entries(templates).map(([kind,content])=><article className="template-card" key={kind}><h3>{names[kind]}</h3><p>{content}</p><button onClick={()=>onApply(kind,content)}>应用</button><button disabled={!ready} onClick={()=>setDraft({id:newId(),name:names[kind]+' · 我的模板',kind,content,version:0})}>复制并编辑</button></article>)}
 </>;
}
