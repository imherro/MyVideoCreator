...
type Transition = { label: string; phase: "loading" | "revealing" };
...
export function useEpisodeTransition() {
  ...
  useEffect(() => {
    // Restore keyboard focus after the workspace is no longer inert.
...
export function EpisodeTransition({ transition }: { transition: Transition | null }) {
  if (!transition) return null;
  // The portal stays outside the temporarily inert workspace.
  return createPortal(
