...
def _director_image_timeout_seconds() -> float:
    """Return the no-progress timeout for Director image jobs.

    Large Flux/Klein image models can take longer than ten minutes on a
    12 GB GPU when weights are streamed through system RAM. Keep this
    configurable so a slow machine can wait without changing the code.
    """
    try:
        value = float(os.environ.get("MAESTRO_IMAGE_GENERATION_TIMEOUT", "3600"))
...
def _invalid_saved_media_numbers(
    ...
    """Return 1-based slots without a non-empty media file under ``output_dir``.

    Normal generated clips are stored as basenames in the output directory.
    Reviewed Director revisions can legitimately reference an owned asset in
    ``_director_assets/<revision>/`` using an absolute path, however.  Treat
    those paths as valid when they remain inside the workspace output root;
    rejecting them made a prepared revision fail before video generation.
    """
    ...
    for index in range(expected_count):
        ...
        try:
            inside_output = (
                os.path.commonpath([
                    os.path.normcase(candidate), normalized_root,
                ]) == normalized_root
            )
        except ValueError:
            inside_output = False
...
def _rerun_clip_image_impl(out_dir: str, pid: str, clip_index: int, prompt_override: str = None) -> dict:
    ...
    output_files = _submit_and_wait(
        gen_params,
        timeout_s=_director_image_timeout_seconds(),
    ...
    try:
        ...
        if reused_plan:
            ...
            try:
                ...
                characters = params.get("characters", []) or []
                from services.director.planners.short_film import _detect_source_language
                _planning_source_text = str(scene_description or "") + "\n" + "\n".join(
                    str(line.get("text") or "")
                    for line in (params.get("lyrics") or [])
                    if isinstance(line, dict)
                )
                _planning_source_language = _detect_source_language(
                ...
                if polish_video_prompts or polish_image_prompts:
                    clip_plans = polish_prompts_third_pass(
                        ...
                        source_language=_planning_source_language,
                    )
        ...
        # Third-pass polish is an independent LLM call and can accidentally
        # remove the source-aware casting anchor added by the planner. Reapply
        # the deterministic Chinese default after polish, before any render
        # starts, so a generic "man/woman" cannot silently become Western.
        if pipeline_type == "short_film_story" and clip_plans:
            try:
                from services.director.planners.short_film import (
                    _detect_source_language,
                    _inject_chinese_casting_default,
                )
                _story_text = str(scene_description or "")
                _story_text += "\n" + "\n".join(
                    str(line.get("text") or "")
                    for line in (params.get("lyrics") or [])
                    if isinstance(line, dict)
                )
                _source_lang = _detect_source_language(_story_text)
                if _source_lang.startswith("zh"):
                    for _plan in clip_plans:
                        for _field in (
                            "image_prompt", "video_prompt",
                        ):
                            _plan[_field] = _inject_chinese_casting_default(
                                _plan.get(_field), _source_lang,
                            )
                        for _field in ("window_prompts", "keyframe_prompts"):
                            _values = _plan.get(_field) or []
            ...
            except Exception as _source_casting_error:
                print(
                    f"[Pipeline {pid}] Source-aware casting pass skipped: "
                    f"{_source_casting_error}"
                )
