...
# HTTP read timeout for local llama-server generations. Large local models can
# spend several minutes on one structured Director pass, especially when the
# model is mostly CPU/RAM-offloaded. Keep the connection timeout short so a
# dead server fails quickly, but allow a long configurable read window.
def _llm_http_timeout() -> tuple[float, float]:
    """Return (connect, read) timeouts for local LLM requests.

    ``MAESTRO_LLM_READ_TIMEOUT`` is intentionally an environment variable so
    users can tune it without editing Python. The default is one hour; the
    old hard-coded value was 600 seconds and was too short for large models.
    """
    try:
        read_timeout = float(os.environ.get("MAESTRO_LLM_READ_TIMEOUT", "3600"))
    except (TypeError, ValueError):
        read_timeout = 3600.0
    return (10.0, max(60.0, read_timeout))
...
def generate_streaming(
    ...
    try:
        ...
        resp.raise_for_status()
        # llama-server returns ``text/event-stream`` without a charset.  The
        # requests library then defaults ``decode_unicode=True`` to
        # ISO-8859-1, turning UTF-8 Chinese into mojibake (``æ...``).  Force
        # the wire encoding before iterating SSE lines.
        resp.encoding = "utf-8"
