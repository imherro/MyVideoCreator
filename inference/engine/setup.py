...
def get_gpu_info():
    ...
    if IS_WIN:
        ...
        try:
            # lspci needs a pipe to grep — use Python's filtering instead
            # of shell=True so we don't feed untrusted data to a shell.
            full = subprocess.check_output(
                ["lspci"],
                encoding='utf-8',
                stderr=subprocess.DEVNULL,
            )
            name = "\n".join(line for line in full.splitlines() if "vga" in line.lower())
