...
def process_files_def(repoId=None, sourceFolderList=None, fileList=None, targetFolderList=None):
    """Download a list of HuggingFace files into the local ckpts tree.
    ...
    Maestro's wgp.py has its own copy of this helper for legacy callers; this
...
def send_download_status(send_cmd=None, status_text=None):
    # Ported from upstream Wan2GP — used by postprocessing bridges (FlashVSR)
    # to surface a "downloading model files" status message before a download.
