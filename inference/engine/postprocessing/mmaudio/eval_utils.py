...
large_44k_v2 = ModelConfig(model_name='large_44k_v2',
                        ...
                        bigvgan_16k_path=None,
                        mode='44k')
large_44k_nsfw = ModelConfig(model_name='large_44k_nsfw',
                        model_path=Path('./weights/mmaudio_large_44k_nsfw_gold_8.5k_final_fp16.safetensors'),
                        vae_path=Path(fl.locate_file('mmaudio/v1-44.pth')),
                        bigvgan_16k_path=None,
                        mode='44k')
...
def generate(
    ...
    *,
    ...
    if clip_video is not None:
        ...
        clip_features = net.get_empty_clip_sequence(bs).to(device, dtype)
    ...
    if sync_video is not None and not image_input:
        ...
        sync_features = net.get_empty_sync_sequence(bs).to(device, dtype)
    ...
    # Scale text conditioning strength via the global conditioning path (text_f_c).
    # text_f_c feeds into global_c = global_cond_mlp(clip_f_c + text_f_c) which controls
    # AdaLN modulation across all transformer blocks — the primary text influence channel.
    # (text_f sequence features go through LayerNorm in joint attention, so scaling them is ineffective.)
    if text_weight != 1.0:
        preprocessed_conditions.text_f_c = preprocessed_conditions.text_f_c * text_weight
