...
def video_to_audio(video, prompt: str, negative_prompt: str, seed: int, num_steps: int,
                   cfg_strength: float, duration: float, save_path , persistent_models = False, audio_file_only = False, verboseLevel = 1, model_name = None, model_path = None, text_weight: float = 1.0):
    """Generate audio from video + text, or text-only when video is None.

    When video is None, generates audio purely from the text prompt and duration.
    In text-only mode, audio_file_only is forced True (no video to remux into).
    """
    ...
    if video:
        # Video-guided generation: extract visual features from video
        ...
        # Text-only generation: use empty visual features
        clip_frames = None
        sync_frames = None
        video_info = None
        audio_file_only = True  # No video to remux into
