...
def _prepare_conditioning_range(sample: torch.Tensor, start: int, end: int, output_height: int, output_width: int, padded_output_height: int, padded_output_width: int, dtype: torch.dtype = torch.bfloat16, device: torch.device | str | None = None) -> torch.Tensor:
    ...
    # Move the SMALL source-resolution slice to the GPU before the bicubic
    # upscale to output resolution. Doing the interpolate on CPU was a major
    # throughput killer: full-output-res float bicubic for every 4-frame
    # chunk, serialized between GPU steps (GPU sat at 50-85% while the CPU
    # churned). On-device the resize is trivial and only ~MBs cross the bus.
...
def _maybe_empty_cache(min_free_gb: float = 3.0) -> None:
    """empty_cache only under actual VRAM pressure.

    The original loop called torch.cuda.empty_cache() twice per process
    step — ~1,600 synchronizing allocator flushes on a 4-minute video —
    which serialized the GPU for nothing on cards with headroom (FlashVSR
    peaks at ~6-7 GB). Keep the safety valve for genuinely tight cards.
    """
    ...
    try:
        ...
        return
    if free_bytes < min_free_gb * (1 << 30):
        torch.cuda.empty_cache()


def _frames_to_uint8(frames: torch.Tensor) -> torch.Tensor:
    """[-1,1] float frames -> uint8 (on the tensor's current device)."""
    if frames.dtype == torch.uint8:
        return frames
    return frames.float().clamp_(-1.0, 1.0).add_(1.0).mul_(127.5).round_().clamp_(0, 255).to(torch.uint8)
...
class FlashVSRRuntime:
    ...
    def _decode_tcdecoder(self, latents: torch.Tensor, sample: torch.Tensor, lq_start: int, lq_end: int, output_height: int, output_width: int, padded_output_height: int, padded_output_width: int, tile_size: int, tile_mems: dict[tuple[int, int], Any] | None, abort_callback=None, progress_callback=None, progress_step: int | None = None, progress_total: int | None = None) -> tuple[torch.Tensor | None, dict[tuple[int, int], Any] | None]:
        ...
        cur_lq = _prepare_conditioning_range(sample, lq_start, lq_end, output_height, output_width, padded_output_height, padded_output_width, dtype=self.dtype, device=self.device).unsqueeze(0)
        ...
        for process_idx in tqdm(range(process_total), desc="FlashVSR"):
            ...
            if process_idx == 0:
                for inner_idx in range(first_chunk_lq_steps):
                    if _abort_requested(abort_callback):
                        return abort_result()
                    lq_chunk = _prepare_conditioning_range(sample, max(0, inner_idx * 4 - 3), (inner_idx + 1) * 4 - 3, output_height, output_width, padded_output_height, padded_output_width, dtype=self.dtype, device=self.device).unsqueeze(0).to(self.device, dtype=self.dtype)
                    lq_list = [lq_chunk]
                    del lq_chunk
                ...
                for inner_idx in range(2):
                    ...
                    lq_chunk = _prepare_conditioning_range(sample, lq_start, lq_start + 4, output_height, output_width, padded_output_height, padded_output_width, dtype=self.dtype, device=self.device).unsqueeze(0).to(self.device, dtype=self.dtype)
                    lq_list = [lq_chunk]
                    del lq_chunk
                    cur = self.lq_proj.stream_forward(lq_list)
                    if cur is not None:
            ...
            if self.variant == FLASHVSR_VARIANT_TINY_LONG:
                ...
                if cur_frames is None:
                    return abort_result()
                # Convert to uint8 ON the GPU before the CPU transfer — the
                # same output contract the "full" variant already uses
                # (decode_to_cpu_uint8). 4x less PCIe traffic per step and a
                # 4x smaller accumulation buffer (which is what bounds chunk
                # sizes for long videos). Debug-video capture needs the float
                # frames, so it converts after saving.
                if not save_still_debug_video:
                    cur_frames = _frames_to_uint8(cur_frames)
                ...
                if copy_frames > 0:
                    if frames_out is None:
                        frames_out = torch.empty((cur_frames.shape[1], input_frames, output_height, output_width), dtype=torch.uint8, device="cpu")
                    frames_out[:, frames_cursor:frames_cursor + copy_frames].copy_(cur_frames[0, :, :copy_frames])
