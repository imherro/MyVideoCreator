...
> **What this repository is:** [Maestro](https://github.com/Blizaine/Maestro)'s
> vendored, import-path-adapted copy of [seed-vc](https://github.com/Plachta/seed-vc)
> by Plachta, distributed separately under its original **GPL-3.0** license
> (see [LICENSE](LICENSE)). Maestro's installer clones this repository into
> `app/postprocessing/seedvc/` at install time — it is not tracked inside the
> Maestro source tree, so the GPL-licensed component keeps its own license and
> distribution channel.
>
> Changes vs upstream seed-vc are limited to packaging: package-relative
> imports, a thin loader adapter in `__init__.py` (asset downloads routed
> through Maestro's checkpoint manager), and CLI entry shims (`_entry.py`,
> `_paths.py`, `api.py`). Model behavior is upstream's. Python dependencies
> are provided by Maestro's environment; this repo is not useful standalone —
> use [upstream seed-vc](https://github.com/Plachta/seed-vc) for that.
>
> Versioned by tags (`v1.0.0`, ...); Maestro's install/update scripts pin a tag.
