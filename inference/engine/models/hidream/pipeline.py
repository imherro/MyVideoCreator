...
def generate_image(
    ...
    # HiDream requires both dimensions divisible by PATCH_SIZE (32). Maestro's
    ...
    # branches below override with find_closest_resolution() or
    # resize_pilimage() which already snap correctly.
    width = max(PATCH_SIZE, (int(width) // PATCH_SIZE) * PATCH_SIZE)
    height = max(PATCH_SIZE, (int(height) // PATCH_SIZE) * PATCH_SIZE)
