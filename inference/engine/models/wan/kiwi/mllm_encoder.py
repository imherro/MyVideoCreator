...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        second_per_grid_ts (`torch.Tensor` of shape `(num_videos)`, *optional*):
            The time interval (in seconds) for each grid along the temporal dimension in the 3D position IDs.
        learnable_query (`torch.FloatTensor` of shape `(num_query_tokens, hidden_size)`, *optional*):
            Optional learnable query features injected at `<|query_pad|>` placeholders before the decoder forward pass.
        """
...
class MLLMEncoder(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if ref_image:
            if not self.any_ref:
                raise ValueError("ref_image was provided but this Kiwi MLLM variant has no reference branch.")
