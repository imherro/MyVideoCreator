...
#
# Adapted for Maestro from zai-org/SCAIL-2's dedicated ``wan-scail2``
# transformer (Apache-2.0).  Keep this model isolated from Maestro's
# generalized WanModel: the reference, pose, mask, and continuation-history
# token regions intentionally have different positional semantics.
import math
...
from .scail2_attention import attention as flash_attention
...
class SCAIL2Model(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.eps = eps
        self.scail2 = bool(scail2)
        # WanAny2V checks this optional cache before denoising.  The official
        # SCAIL-2 path deliberately does not use WanGP's generic Tea/MagCache
        # shortcuts because they alter the dedicated token stream.
        self.cache = None
    ...
    @staticmethod
    def preprocess_sd_with_dtype(dtype, state_dict):
        """Map Maestro's compact SCAIL-2 checkpoint into upstream names."""
        ...
        for key, value in state_dict.items():
            for prefix in ("model.diffusion_model.", "diffusion_model."):
                if key.startswith(prefix):
                    key = key[len(prefix):]
                    break
            if key.startswith("pose_patch_embedding."):
                key = key.replace(
                    "pose_patch_embedding.", "patch_embedding_pose.", 1,
                )
            elif key.startswith("mask_patch_embedding."):
                key = key.replace(
                    "mask_patch_embedding.", "patch_embedding_mask.", 1,
                )
            if not key.startswith("vae."):
                converted[key] = value
        return converted

    def preprocess_loras(self, base_model_type, state_dict):
        """Retain Wan LoRA compatibility without generic-model-only tensors."""

        converted = {}
        for key, value in state_dict.items():
            if (
                "patch_embedding.diff" in key
                and torch.is_tensor(value)
                and value.ndim >= 2
                and value.shape[1] != self.in_dim
            ):
                continue
            if key.endswith("modulation.diff"):
                continue
            converted[key.replace(".default.", ".")] = value
        return converted

    def apply_post_init_changes(self):
        """Compatibility hook used by WanAny2V after model loading."""

    def lock_layers_dtypes(self, dtype=torch.float32):
        """Preserve the float32 islands used by the official model loader."""
        ...
        primary_layers = [
            self.head,
            self.head.head,
            self.head.modulation,
            self.patch_embedding,
            self.patch_embedding_pose,
        ...
        precision_layers.extend(block.norm3 for block in self.blocks)

        seen = set()
        for layer in primary_layers + precision_layers:
            if id(layer) in seen:
                continue
            seen.add(id(layer))
            ...
            if torch.is_tensor(weight) and not isinstance(weight.data, QTensor):
                weight.data = weight.data.to(dtype)
            bias = getattr(layer, "bias", None)
            if torch.is_tensor(bias) and not isinstance(bias.data, QTensor):
                bias.data = bias.data.to(dtype)
        self._lock_dtype = dtype

    @staticmethod
    def _as_sample_list(value, *, name):
        """Normalize Maestro's batched tensors to upstream sample lists."""
        ...
        for tensor in values:
            ...
            if tensor.ndim == 4:
                ...
                samples.extend(tensor.unbind(0))
            else:
                raise ValueError(
                    f"{name} tensors must be 4D or 5D, got {tensor.ndim}D",
                )
        return samples

    @staticmethod
    def _as_context_list(context):
        values = context if isinstance(context, (list, tuple)) else [context]
        samples = []
        for tensor in values:
            if not torch.is_tensor(tensor):
                raise TypeError("context must contain tensors")
            if tensor.ndim == 2:
                samples.append(tensor)
            elif tensor.ndim == 3:
                samples.extend(tensor.unbind(0))
                ...
                raise ValueError(
                    f"context tensors must be 2D or 3D, got {tensor.ndim}D",
                )
        return samples

    @staticmethod
    def _match_sample_count(samples, count, *, name):
        if samples is None or len(samples) == count:
            return samples
        if len(samples) == 1:
            return samples * count
        raise ValueError(
            f"{name} supplied {len(samples)} sample(s) for batch size {count}",
        )
    ...
    def forward(
        self,
        x: list[torch.Tensor],
        pose_latents: list[torch.Tensor]=None,
        driving_masks: list[torch.Tensor]=None,
        ref_latents: list[torch.Tensor]=None,
        ref_masks: list[torch.Tensor]=None,
        t=None,
        context=None,
        seq_len=1e10,
        replace_flag: bool=False,
        history_mask: list[torch.Tensor]=None,
        clip_fea=None,
        ...
        scail2_history_mask=None,
        scail2_additional_ref_latents=None,
        scail2_additional_ref_masks=None,
        scail2_replace_flag=None,
        **unused_kwargs,
    ):
        r"""
        ...
        ref_latents = (
            scail2_ref_latents
            if scail2_ref_latents is not None
            else ref_latents
        )
        ref_masks = (
            scail2_ref_masks
            if scail2_ref_masks is not None
            else ref_masks
        )
        history_mask = (
            scail2_history_mask
            if scail2_history_mask is not None
            else history_mask
        )
        additional_ref_latents = (
            scail2_additional_ref_latents
            if scail2_additional_ref_latents is not None
            else additional_ref_latents
        )
        additional_ref_masks = (
            scail2_additional_ref_masks
            if scail2_additional_ref_masks is not None
            else additional_ref_masks
        )
        if scail2_replace_flag is not None:
            replace_flag = bool(scail2_replace_flag)
        ...
        if missing:
            raise ValueError(
                "Dedicated SCAIL-2 conditioning is missing: "
                + ", ".join(missing),
            )

        original_x = x if isinstance(x, (list, tuple)) else [x]
        output_batch_sizes = [
            tensor.shape[0] if tensor.ndim == 5 else None
            for tensor in original_x
        ]
        x = self._as_sample_list(x, name="x")
        batch_size = len(x)
        pose_latents = self._match_sample_count(
            self._as_sample_list(pose_latents, name="pose_latents"),
            batch_size,
            name="pose_latents",
        )
        driving_masks = self._match_sample_count(
            self._as_sample_list(driving_masks, name="driving_masks"),
            batch_size,
            name="driving_masks",
        )
        ref_latents = self._match_sample_count(
            self._as_sample_list(ref_latents, name="ref_latents"),
            batch_size,
            name="ref_latents",
        )
        ref_masks = self._match_sample_count(
            self._as_sample_list(ref_masks, name="ref_masks"),
            batch_size,
            name="ref_masks",
        )
        history_mask = self._match_sample_count(
            self._as_sample_list(history_mask, name="history_mask"),
            batch_size,
            name="history_mask",
        )
        additional_ref_latents = self._match_sample_count(
            self._as_sample_list(
                additional_ref_latents,
                name="additional_ref_latents",
            ),
            batch_size,
            name="additional_ref_latents",
        )
        additional_ref_masks = self._match_sample_count(
            self._as_sample_list(
                additional_ref_masks,
                name="additional_ref_masks",
            ),
            batch_size,
            name="additional_ref_masks",
        )
        context = self._match_sample_count(
            self._as_context_list(context),
            batch_size,
            name="context",
        )
        if not torch.is_tensor(t):
            t = torch.as_tensor(t, device=x[0].device)
        if t.ndim == 0:
            t = t.unsqueeze(0)
        if t.numel() == 1 and batch_size > 1:
            ...
            raise ValueError(
                f"t supplied {t.numel()} value(s) for batch size {batch_size}",
            )
        if clip_fea.shape[0] == 1 and batch_size > 1:
            clip_fea = clip_fea.expand(batch_size, *clip_fea.shape[1:])
        elif clip_fea.shape[0] != batch_size:
            raise ValueError(
                "clip_fea batch does not match the SCAIL-2 sample batch",
            )
        ...
        # unpatchify
        x = self.unpatchify(
            x, grid_sizes, offset=additional_ref_length + ref_length,
        )
        x = [u.float() for u in x]
        if any(size is not None for size in output_batch_sizes):
            regrouped = []
            offset = 0
            for size in output_batch_sizes:
                count = 1 if size is None else size
                group = x[offset:offset + count]
                regrouped.append(
                    group[0] if size is None else torch.stack(group, dim=0),
                )
                offset += count
            return regrouped
        return x
