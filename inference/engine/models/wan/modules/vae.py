...
class WanVAE_(nn.Module):
    ...
    def decode_to_cpu_uint8(self, z, scale=None, tile_size=0, target_frames=None, target_height=None, target_width=None, any_end_frame=False, device=None, frame_start=0):
        # Ported from upstream Wan2GP for FlashVSR's "full" variant, which
        # decodes via this VAE (the "tiny" variant uses its own tcdecoder).
        # Decodes the latent, crops to the requested frame range + resolution,
        # and returns CPU uint8 [B,3,T,H,W]. Non-tiled: tile_size > 0 still
        # decodes the whole tensor (correct output, just more VRAM) — FlashVSR
        # uses tile_size=0 for typical resolutions.
...
class WanVAE:
    ...
    def decode_to_cpu_uint8(self, zs, tile_size, target_frames=None, target_height=None, target_width=None, any_end_frame=False, frame_start=0):
        # Used by FlashVSR's "full" variant. Delegates to the inner module's
        # decode_to_cpu_uint8 per latent (returns CPU uint8 [3,T,H,W] each).
        scale = [u.to(device=self.device) for u in self.scale]
        return [
            self.model.decode_to_cpu_uint8(u.to(self.dtype).unsqueeze(0), scale, int(tile_size or 0), target_frames=target_frames, target_height=target_height, target_width=target_width, any_end_frame=any_end_frame, device=self.device, frame_start=frame_start).squeeze(0)
            for u in zs
        ]
