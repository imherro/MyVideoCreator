    ...
    return {
        ...
        "audio_prompt_type_sources": {
            ...
            "labels": {
                ...
                "AB": "Voice cloning (up to 6 speakers)",
            },
            "letters_filter": "AB",
            "default": "",
        },
        # KugelAudio's modes map 1:1 to voice count (0 → "", 1 → "A", 2-6 → "AB"),
        # so the UI hides the audio-mode ChoiceControl for this model. The voice-
        # count buttons (Add Voice / Remove Voice slots) become the sole source
        # of truth for audio_prompt_type. Phase 6 (19eda0b) added the ChoiceControl
        # to all audio-only models so Scenema could pick between text-only / single-
        # ref / two-ref SeedVC modes — but for Kugel this created two competing
        # sources of truth that could drift apart. Restoring the old "voice-slot
        # buttons drive everything" behavior eliminates the regression.
...
class family_handler:
    ...
    def validate_generative_prompt(base_model_type, model_def, inputs, one_prompt):
        ...
        text = str(one_prompt)
        # Match the actual speaker-tag pattern (e.g. "Speaker 1:") rather than the
        # bare word "speaker" — the old substring check would false-positive on
        # legitimate prose like "the speaker on stage said hello" or any prompt
        # that happens to mention a speaker. Tag pattern is the same as the
        # multi-voice branch below uses (line ~265).
        has_speaker_tags = re.search(r"Speaker\s*\d+\s*:", text, flags=re.IGNORECASE) is not None
        if has_speaker_tags:
        ...
        if "B" in audio_prompt_type:
            # Multi-voice cloning: requires at least 2 reference audio files
            if inputs.get("audio_guide") is None or inputs.get("audio_guide2") is None:
                return "Multi-voice cloning requires at least two reference audio files."
            speaker_matches = list(re.finditer(r"Speaker\s*(\d+)\s*:", text, flags=re.IGNORECASE))
            if not speaker_matches:
                return (
                    "Multi-voice cloning requires prompt lines with Speaker N: tags "
                    "(e.g. Speaker 1:, Speaker 2:). For headless settings, keep "
            ...
            if len(speaker_ids) < 2:
                return (
                    "Multi-voice cloning requires at least 2 speaker IDs. Use Speaker 1: and Speaker 2: "
                    "(up to Speaker 6:). For headless settings, keep 'multi_prompts_gen_type' = 2."
                )
            if len(speaker_ids) > 6:
                return f"KugelAudio supports up to 6 voice clones (found {len(speaker_ids)} speakers)."
            # Validate that we have audio files for each speaker
            for sid in speaker_ids:
                guide_key = "audio_guide" if sid == 1 else f"audio_guide{sid}"
                if inputs.get(guide_key) is None:
                    return f"Speaker {sid} requires a voice reference audio file (audio_guide{sid})."
        return None
