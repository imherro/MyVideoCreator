        ...
        **kwargs,
        ...
        if "Speaker" in text and (audio_guide is not None or audio_guide2 is not None):
            ...
            if parsed and audio_guide2 is not None:
                ...
                if len(unique_ids) > 6:
                    raise ValueError("Multi-speaker mode supports up to 6 speaker IDs.")
                # Map Speaker N → internal index N-1 (Speaker 1→0, Speaker 2→1, etc.)
                speaker_id_map = {sid: i for i, sid in enumerate(unique_ids)}
            if KUGELAUDIO_DEBUG:
            ...
            if parsed:
                ...
                for speaker_id, line_text, append_pause_after in expanded:
                    ...
                    # Select voice reference for each speaker (0=audio_guide, 1=audio_guide2, etc.)
                    voice_map = {
                        0: audio_guide,
                        1: audio_guide2,
                        2: kwargs.get("audio_guide3"),
                        3: kwargs.get("audio_guide4"),
                        4: kwargs.get("audio_guide5"),
                        5: kwargs.get("audio_guide6"),
                    }
                    if mapped_id in voice_map and voice_map[mapped_id] is not None:
                        voice_path = voice_map[mapped_id]
