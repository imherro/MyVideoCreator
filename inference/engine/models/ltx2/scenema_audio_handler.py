...
# seed-vc is GPL-3.0, so it ships as a separately-licensed component that
# install.js/update.js clone into app/postprocessing/seedvc (not tracked in
# this repo). This handler is imported at server startup via wgp's
# family_handlers list, so a missing/failed clone must degrade to a clear
# error at USE time instead of killing the whole server import chain.
try:
    from postprocessing import seedvc
except Exception as _seedvc_error:
    seedvc = None
    _SEEDVC_IMPORT_ERROR = repr(_seedvc_error)
else:
    _SEEDVC_IMPORT_ERROR = None
...
def _require_seedvc():
    if seedvc is None:
        raise RuntimeError(
            "SeedVC voice components are not installed (import failed: "
            f"{_SEEDVC_IMPORT_ERROR}). Run Update in Pinokio to fetch the "
            "seed-vc component (github.com/Blizaine/maestro-seedvc)."
        )
    return seedvc
...
def _get_scenema_model_def():
    return {
        ...
        # Auto-mode: derive audio_prompt_type from voice-clone slot count.
        # 0 voices → "" (text-only), 1 → "A2" (single SeedVC ref), 2+ → "AB2"
        # (two SeedVC refs). User adds/removes voice slots via the UI buttons
        # and the mode follows automatically — matches KugelAudio's UX and
        # eliminates the dual-source-of-truth confusion that came from also
        # exposing the manual ChoiceControl. Extra slots beyond 2 are silently
        # ignored by Scenema (per Phase 6 plan §9 R12).
        "audio_mode_from_voice_count": True,
        # Scenema only consumes the first 2 voice references (A2 / AB2 modes).
        # The UI caps the "Add Voice" button at this number so users aren't
        # offered slots 3-6 that would be silently discarded.
        "max_voice_count": 2,
        ...
        # Force bf16 Gemma for Scenema regardless of the user's global
        # `text_encoder_quantization` setting. Side-by-side test on
        # 2026-05-18 showed: int8 Gemma → Scenema hallucinates dialogue
        ...
        # quality gain. Other Maestro models continue to use the user's
...
def _get_scenema_download_def():
    ...
    ] + _require_seedvc().query_download_def()
...
class family_handler:
    ...
    def load_model(
        ...
        **kwargs,
        ...
        # ---------------------------------------------------------------
        # Maestro / Triton-INT8 defensive workarounds — mirror HiDream port.
        ...
        # remaining divergence is in some other Maestro-runtime layer.
        ...
        try:
            ...
            print(f"[Scenema] Could not disable Triton int8 kernel injection: {e}")
        ...
        try:
            ...
            if _inj._BASE_PATCH_STATE.enabled and _inj._BASE_PATCH_STATE.orig_forward is not None:
                ...
                print("[Scenema] Restored optimum.quanto's true original linear forward (was wrapped by Maestro default-kernel patch)")
        except Exception as e:
        ...
        try:
            ...
            if os.path.isdir(cache_dir):
                print(f"[Scenema] Clearing torch.compile inductor cache at {cache_dir} (avoid stale Triton-kernel graphs)")
                shutil.rmtree(cache_dir, ignore_errors=True)
        except Exception as e:
            print(f"[Scenema] Could not clear torch.compile inductor cache: {e}")
        # ---------------------------------------------------------------
        ...
        seedvc_model = _require_seedvc().get_model(dtype=torch.float16)
    ...
    def fix_settings(base_model_type, settings_version, model_def, ui_defaults):
        ...
        # Maestro uses integer multi_prompts_gen_type (0/1/2/3 — see wgp.py); upstream
        ...
        mpgt = ui_defaults.get("multi_prompts_gen_type")
        if isinstance(mpgt, str):
            ui_defaults["multi_prompts_gen_type"] = 2 if mpgt.upper() == "FG" else 0
        ui_defaults.setdefault("multi_prompts_gen_type", 2)
