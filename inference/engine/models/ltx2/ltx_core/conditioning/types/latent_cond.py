...
class AudioConditionByLatentPrefix(ConditioningItem):
    ...
    Used for sliding-window audio continuity: window N+1 gets window N's trailing
    audio latents as a clean prefix so voice/tone carries over smoothly at the
    join instead of re-generating audio from scratch.
    """
