...
class RopeLayoutCache:
    ...
    # Per-axis precomputed token-index tensor used by the compile-safe rope
    # path. The non-compile-safe path uses `as_strided` slicing, which trips
    # the torch._dynamo proxy-tracking bug on LTX-2 distilled. Building
    # explicit token indices ahead of time lets the compile-safe branch use
    # `index_select` instead, which dynamo handles cleanly. Ported from
    # upstream Wan2GP v11.77 — see `_apply_split_rope_layout_compile_safe`.
...
def _apply_split_rope_layout_compile_safe(input_tensor: torch.Tensor, rope_cache: RopeCache, layout: RopeLayoutCache) -> None:
    """Compile-safe variant of the split-rope inplace layout.

    The non-compile-safe path uses `input_tensor.reshape(b, *grid_sizes,
    num_heads, dim_head)` plus strided slicing. Under torch.compile, dynamo
    trips a proxy-tracking bug on the dynamically-sized reshape:

        RuntimeError: 8*s1*s15 (...) is not tracked with proxy

    This variant keeps the input shape flat ([B, tokens, dim]), reshapes
    only to add the heads dim, and uses precomputed per-axis token indices
    (`layout.axis_token_indices`) with `index_select` to gather per-token
    freqs. Result: identical math, but dynamo handles it cleanly.

    Ported verbatim from upstream Wan2GP v11.77.
    """
...
def _build_layout_axis_token_indices(grid_sizes: tuple[int, ...], axis: int, device: torch.device) -> torch.Tensor:
    """Build a 1D index tensor that maps token-index → axis-value-index.

    For a grid of shape `grid_sizes`, every token has a coordinate along
    each axis. `_apply_split_rope_layout_compile_safe` gathers per-token
    cos/sin freqs by `index_select`-ing these indices.

    Ported from upstream Wan2GP v11.77.
    """
...
def _apply_split_rope_layout_inplace(input_tensor: torch.Tensor, rope_cache: RopeCache, layout: RopeLayoutCache) -> None:
    ...
    if axis_width <= 0 or dim_no_pad <= 0 or dim_no_pad % axis_count != 0 or axis_width * axis_count != dim_no_pad:
        return
    # Dispatch to the compile-safe variant when running inside torch.compile.
    # The non-compile-safe `reshape(b, *grid_sizes, ...)` below trips a
    # torch._dynamo proxy-tracking bug on LTX-2 distilled. Required for
    # ID-LoRA to actually attach via MMGP's compile-based LoRA injection on
    # quanto INT8 audio-attention layers — see ltx2_handler.py compile note.
    if _is_compiling_graph():
...
def _infer_grid_sizes(positions: torch.Tensor) -> tuple[list[torch.Tensor], tuple[int, ...]]:
    # Cast to float for compatibility with environments where torch.unique
    # lacks bf16 support (silently mishandles negative values from Scenema's
    # AudioConditionByReferenceLatent → wrong rope encoding for ref tokens →
    # speakers repeat prior chunks' content). Ported from upstream Wan2GP.
...
def _make_layout(
    ...
    # Per-axis token-index tensors needed by the compile-safe rope path.
    # Built eagerly here so the dispatch in _apply_split_rope_layout_inplace
    # → _apply_split_rope_layout_compile_safe can index without materializing
    # tensors inside the compiled region.
