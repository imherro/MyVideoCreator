...
class DiffusionVideoDecoder(nn.Module):
    ...
    def __init__(
        ...
        upsamples: Sequence[Sequence],
        stage5_kernel: Sequence[int],
        ...
        super().__init__()
        print(
            "LTX-2.5 NAD neighborhood attention backend: "
            f"{_attention_backend_message()}"
        )
        ...
        self.stage_kernels = tuple(
            tuple(int(axis) for axis in kernel) for kernel in stage_kernels
        )
                ...
                nn.ModuleList(
                    [
                        NABlock(
                            ...
                            self.stage_kernels[index],
                            head_dim,
                        )
                        for _ in range(self.stage_depths[index])
                    ]
                )
            )
        ...
        self.conv_in_x_t = ChannelLinear(
            noised_channels,
            final_channels,
            bias=True,
        )
