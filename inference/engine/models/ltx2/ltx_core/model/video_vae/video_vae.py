...
def decode_video_to_tensor(
    ...
    # Allocate the output buffer outside torch.inference_mode so later code
    # paths (e.g. outpainting gamma roundtrip in ltx2.py) can mutate it in
    # place without "tensor is in inference mode" errors. Per upstream WanGP
    # commit 5da7f23 ("unlocked ltx2 dev, added image post processing").
    ...
    try:
        if tiling_config is not None:
            ...
            if is_diffusion_decoder:
                tiled_decode_kwargs["output_uint8"] = True
                decoder_input = [latent]
                latent = None
                tiled_iterator = video_decoder.tiled_decode(
                    decoder_input,
            ...
            for frames in tiled_iterator:
                ...
                if frames.dtype is not torch.uint8:
