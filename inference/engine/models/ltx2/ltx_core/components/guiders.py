...
class MultiModalGuiderParams:
    """Parameters for MultiModalGuider — used by DramaBox TTS pipeline.

    Ported from upstream WanGP for DramaBox audio generation. Combines
    CFG, STG, modality, and rescale guidance terms in a single per-step
    update.
    """
    cfg_scale: float = 1.0
    stg_scale: float = 0.0
...
class MultiModalGuider:
    """Composite guider that combines CFG + STG + modality + rescale terms.

    Used by DramaBox TTS pipeline (see app/models/ltx2/ltx_audio_tts.py).
