...
class ICLoraPipeline:
    ...
    def __call__(
        ...
        if callback is not None:
            # Pre-declare total across both stages so the progress bar tracks
            # cumulative progress instead of resetting at pass 2. Stage 2 uses
            # the fixed STAGE_2_DISTILLED_SIGMA_VALUES schedule.
            _pipeline_total_steps = (len(stage_1_sigmas) - 1) + (len(STAGE_2_DISTILLED_SIGMA_VALUES) - 1)
