...
class ScenemaAudioPipeline:
    ...
    def generate(
        ...
        *,
        ...
        verbose_level: int = 0,
        # Maestro forwards audio_guide3..audio_guide6 (UI exposes 6 voice slots);
        ...
        **_unused_extra_voice_slots,
        ...
        self._early_stop = False
        # ---------------------------------------------------------------
        # Per-generation re-application of the HiDream-style Triton/quanto
        # defensive workarounds. The load_model variants run only on first
        # load; if the user switches to another model that re-enables
        # Triton (e.g. LTX-2 video) and then comes back to Scenema, the
        # model object is reused but the global Triton/quanto state has
        # been re-patched in between. Re-disable + re-restore on every
        # generate so Scenema always runs against the TRUE optimum.quanto
        # forward. Symbol-identical to wan.git2 with enable_int8_kernels=0.
        # See commits cbc3118, ee0cd47 for full rationale.
        # ---------------------------------------------------------------
        try:
            ...
            disable_quanto_int8_kernel(notify_disabled=False)
